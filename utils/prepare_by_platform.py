...
from custom_quantizer.lp_academic_quantizer import replace_lp_academic_quantizer
    ...
    for node in graph.nodes:
        if node.op == 'call_module':
            ...
            if isinstance(module, MessagePassing):
                # found a MessagePassing, replace its Linear and Sequentials
                logger.info("Found a MessagePassing @ %s, replace its Linear" % (node.target,))
                # TODO: Why GAT.named_modules can not get all submodules?
                ...
                for k in sub_modules.keys():
                    ...
                    if isinstance(v, torch_geometric.nn.dense.Linear):
                        # replace pyg.nn.dense.Linear -> torch.nn.Linear
